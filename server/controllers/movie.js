    ...
    try {
      ...
      if (category) {
        if (type === "top_rated") {
          ...
          const movies = await Movie.find({ category, viewed: { $gte: 5 } })
            .sort({ viewed: -1 })
            .skip(limit * currPage - limit)
            .limit(limit);
          data = movies;
          ...
          const date = new Date();
        // Lấy ngày đầu tiên của tháng hiện tại (YYYY-MM-01)
          const year = date.getFullYear();
          const month = String(date.getMonth() + 1).padStart(2, "0"); // vì getMonth() = 0–11
