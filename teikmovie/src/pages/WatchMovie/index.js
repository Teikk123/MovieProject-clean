...
function WatchMovie() {
    ...
    let src = `https://vidsrc.me/embed/${category}/${id}`;
    if (category === 'tv') {
        src += `?s=${movieDetail?.seasons ?? 1}&e=${searchParams.get('episode') ?? 1}`;
    }

    // fallback sang youtube trailer nếu vidsrc lỗi
    const getEmbedUrl = () => {
        if (src) {
            return src; // ưu tiên vidsrc
        }
        if (movieDetail?.trailerCode) {
            return `https://www.youtube.com/embed/${movieDetail.trailerCode}`;
        }
        return "";
    };
