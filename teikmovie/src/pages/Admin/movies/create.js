...
import Select from 'react-select'; // ✅ thêm thư viện react-select
...
const CreateMovie = () => {
    ...
    const Onsubmit = async (data) => {
        ...
        // ✅ genres (react-select) trả về array object → convert thành array id
        if (data.genres) {
            data.genres = data.genres.map((g) => g.value);
        }
    ...
    const handleUploadImg = async (e) => {
        ...
        formData.append("upload_preset", "MovieWeb"); 

        try {
            const res = await fetch(
                "https://api.cloudinary.com/v1_1/dh5gvbozr/image/upload",
                {
                    method: "POST",
                    body: formData,
                }
